...
// --- SESSION CONFIG ---
const sessionOption = {
    secret: process.env.SECRET || "devsecret", // Uses Render ENV, defaults to 'devsecret'
    resave: false,
    saveUninitialized: true,
    cookie: {
        expires: Date.now() + 7 * 24 * 60 * 60 * 1000,
...
app.post("/listing", isLoggedIn, upload.single('avatar'), async (req, res, next) => {
    try {
        ...
        let latitude = (responsee.data && responsee.data.length > 0) ? parseFloat(responsee.data[0].lat) : 28.4744;
        let longitude = (responsee.data && responsee.data.length > 0) ? parseFloat(responsee.data[0].lon) : 77.5040;
...
app.put("/listing/:id", isLoggedIn, upload.single("avatar"), async (req, res) => {
    let { id } = req.params;
    let listingData = await listing.findById(id);
    if (!listingData.owner.equals(req.user._id)) return res.send("Permission denied");
...
app.delete("/listing/:id/reviews/:reviewId", isLoggedIn, async (req, res) => {
    let { id, reviewId } = req.params;
    let review = await Reviews.findById(reviewId);
    if (!review.author.equals(req.user._id)) return res.send("Permission denied");
    await Reviews.findByIdAndDelete(reviewId);
...
app.get("/logout", (req, res) => {
    req.logout((err) => { if (err) return next(err); res.redirect("/listing"); });
});
